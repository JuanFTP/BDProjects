...
/*Inserccion*/
	...
	public boolean validarParaInsercion(JFrame window, int folio, String direccion, String tipo, String estado, int id, String userUpper)
	{
		this.window = window;
		respuesta=false;
		...
		{
			if(!esDuplicado(descripcion, apellidos, telefono))
			{
				String query = "insert into usuario (descripcion, apellidos, direccion, telefono, login, fecha) values ('"+descripcion+"', '"+apellidos+"', '"+direccion+"', '"+telefono+"', '"+userUpper+"', curdate());";
	...
	public boolean esDuplicado(String descripcion, String apellidos, String telefono)
	{
		boolean r=false;
		try
		{
			conector = new enlace(); fix= conector.getConnection();
			consulta = fix.createStatement();
			resultado = consulta.executeQuery("select count(*) from usuario where descripcion='"+descripcion+"' and apellidos='"+apellidos+"';");
			if(resultado.next())
			{
				...
				{
					if(descripcion.equals("modificar")) /*Para que extraiga todo menos el mismo telefono del usuario si en caso este lo deja igual*/
